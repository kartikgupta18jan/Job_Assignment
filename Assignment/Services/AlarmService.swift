...
class AlarmService
{
    ...
    private(set) public var nextAlarm: AlarmEntity? = nil
    ...
    {
        CoreDataService.instance.loadEntities(ofType: AlarmEntity.self) { (inAlarms) in
    ...
    func makeNewAlarmAndSave(onComplete: @escaping (_ newAlarm: AlarmEntity?) -> Void)
    {
        let newAlarm = CoreDataService.instance.makeNewEntity(ofType: AlarmEntity.self)
        ...
        CoreDataService.instance.saveAllEntities { (success) in
            ...
            {
                CoreDataService.instance.deleteEntity(ofType: AlarmEntity.self, entity: newAlarm)
    ...
    func deleteAlarmAndSave(_ alarm: AlarmEntity, onComplete: @escaping (_ success: Bool) -> Void)
    {
        CoreDataService.instance.deleteEntity(ofType: AlarmEntity.self, entity: alarm)
    ...
    func scheduleSnooze(forAlarm alarm: AlarmEntity)
    {
        self.cancelNextAlarm()
    ...
    func scheduleAlarmNotification()
    {
        ...
        content.sound = UNNotificationSound(named: UNNotificationSoundName(rawValue: "\(self.nextAlarm?.soundName ?? "alarm_digital").wav"))
    ...
    private func getNextEnabledWeekday(forAlarm alarm: AlarmEntity, startingFrom initialWeekday: AlarmUtility.Weekday) -> AlarmUtility.Weekday?
    {
