...
class ModifyAlarmViewController: UITableViewController, RingingAlarmLaunchable{
    ...
    override func viewDidLoad() {
        ...
        self.lblSnoozeTime.text = "\((self.alarm.snoozeDuration)) min"
    ...
    func loadAlarm(_ passedAlarm: AlarmEntity){
    ...
    private func onAlarmFiredOff(_ notification: Notification){
        let alarm = notification.userInfo!["Alarm"] as! AlarmEntity
    ...
    @IBAction func onDoneBtnPressed(_ sender: Any) {
        dismiss(animated: true, completion: {
            self.delegateDone?.alarmDataPass()
        })
    }
...
extension ModifyAlarmViewController: UIPickerViewDelegate, UIPickerViewDataSource
{
    ...
    {
        let attributedString = NSAttributedString(string: self.alarmSoundNames[row], attributes: [NSAttributedString.Key.foregroundColor : UIColor.white])
